...
#if Unity_ANDROID
        fileAndroid = Path.Combine("\storage\emulated\0\documents","session_log.csv");
         if (!File.Exists(fileAndroid))
        {
            File.WriteAllText(fileAndroid, "SessionID, BeginTime, EndTime, SessionDuration, AdminID, PacientID, CompletedLevels, LevelsData, Commentary\n");
        }
#endif
...
#if UNITY_ANDROID //Salva no Android
        File.AppendAllText(fileAndroid, sessionData);
#endif
